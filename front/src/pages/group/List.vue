<template>
  <q-page padding>
    ...
    <div class="row justify-center" v-if="groups.length > 0">
      <q-pagination
    ...
    >
      <q-btn fab icon="mdi-plus" color="primary" :to="{ name: 'form-group' }" />
    </q-page-sticky>
  </q-page>
</template>
...
export default defineComponent({
  ...
  setup() {
    const { list, remove } = groupService();
    ...
    const handleEdit = (entity) => {
      router.push({ name: "form-group", params: { id: entity.uuid } });
    };
    ...
    return {
      ...
      pagesNumber: computed(() =>
        Math.ceil(groups.value.length / initialPagination.value.rowsPerPage)
      ),
    };
  },
});
