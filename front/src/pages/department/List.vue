<template>
  <q-page padding>
      ...
      >
        <template v-slot:top>
          <span class="text-h6" v-if="$q.platform.is.desktop"
            >Listagem de Departamentos</span
          >
          <q-space />
          <q-input
            outlined
    ...
    <div class="row justify-center" v-if="departments.length > 0">
      <q-pagination
...
export default defineComponent({
  name: "PageListDepartments",

  setup() {
    const { list, remove } = departmentService();
    ...
    const handleEdit = (entity) => {
      router.push({ name: "form-department", params: { id: entity.uuid } });
    };
    ...
    return {
      ...
      pagesNumber: computed(() =>
        Math.ceil(
          departments.value.length / initialPagination.value.rowsPerPage
        )
      ),
    };
  },
