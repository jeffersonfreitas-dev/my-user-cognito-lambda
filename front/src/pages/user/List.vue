<template>
  <q-page padding>
    ...
    <div class="row justify-center" v-if="users.length > 0">
      <q-pagination
    ...
    >
      <q-btn fab icon="mdi-plus" color="primary" :to="{ name: 'form-user' }" />
    </q-page-sticky>
  </q-page>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleEdit = (entity) => {
      router.push({ name: "form-user", params: { id: entity.uuid } });
    };

    const handleDetail = (entity) => {
      router.push({ name: "datail-user", params: { id: entity.uuid } });
    };
    ...
    return {
      ...
      pagesNumber: computed(() =>
        Math.ceil(users.value.length / initialPagination.value.rowsPerPage)
      ),
    };
  },
});
